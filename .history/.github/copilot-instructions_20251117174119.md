...
VS Code: Tasks & Vorschau (praktisch)
- Verfügbare Tasks (VS Code):
  - `Start Project (start.sh)` — führt `./scripts/start.sh` aus (installiert ggf. und startet pnpm oder nutzt Docker-Fallback).
  - `Start Dev Server` — startet `pnpm dev` (HMR via Vite).
  - `Start Dev Server + Open` — startet den Dev-Server (falls nötig) und öffnet die vom Script erkannte URL (prüft Ports `3000`–`3020`).
