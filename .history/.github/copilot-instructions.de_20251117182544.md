...
- Skripte: `scripts/start.sh`, `scripts/start-dev-and-open.sh`, `scripts/open-dev-url.sh`.
 - Skripte: `scripts/start.sh`, `scripts/start-dev-and-open.sh`, `scripts/open-dev-url.sh`.
 - Automation assisten: `scripts/assistant_automate.sh` — interaktives Hilfs-Skript, das 10s auf eine Aufgabenliste wartet und bei Timeout eine Standardfolge ausführt; Checkpoints werden in `tmp_debug/assistant_checkpoints.md` geschrieben.
...
Automation assisten
- **Zweck**: Kleines Automationsskript, das per Eingabe oder nach 10s Timeout eine vordefinierte Aufgabenfolge ausführt und Ergebnisse/Status in `tmp_debug/assistant_checkpoints.md` protokolliert.
- **Ausführen**: VS Code Task `Automation Assisten` oder `bash ./scripts/assistant_automate.sh`.
- **Hinweis**: Lang laufende `dev`-Befehle werden heuristisch im Hintergrund gestartet (PID-Log) damit die restlichen Schritte weiterlaufen können.
