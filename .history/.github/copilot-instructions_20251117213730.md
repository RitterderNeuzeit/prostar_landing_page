...
# AI-Copilot Instructions — concise & actionable

This file gives the essential, repo-specific knowledge an AI coding agent needs to be productive. Keep it short; follow links below for deeper docs.

English / Deutsch (kurz)

- Architecture: SPA frontend (Vite) in `client/` + Express + tRPC backend in `server/`. DB via Drizzle (`drizzle/`). Stripe routes under `server/routes/stripe`. Package manager: `pnpm`.
...
- Critical, non-obvious rules (do not break)
