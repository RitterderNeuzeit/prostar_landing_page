...
const DEV_PORT = Number((new URL(process.env.ADMIN_URL || 'http://localhost:3000')).port || 3000);
...
function startDevServer() {
  ...
  const child = spawn('pnpm', ['dev'], { shell: true, detached: true, stdio: 'ignore' });
  child.unref();
  return child.pid;
}
  ...
  try {
    ...
    checkServer(port, ok => {
      if (ok) {
        ...
        pid = startDevServer();
        ensureServerReady(port, 60000).then(() => createWindow()).catch(err => {
