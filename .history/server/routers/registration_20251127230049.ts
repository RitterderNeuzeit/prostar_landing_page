...
import { z } from "zod";
import { registerForCourse, verifyAccessKey, markEmailSent } from "../services/courseServiceAuto";
  ...
  create: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!regResult.success) {
        return regResult as any;
      }

      let emailStatus: { success: boolean; messageId?: string; error?: string } = { success: false };
      if (isEmailServiceConfigured() && regResult.accessKey && regResult.expiresAt) {
        ...
        if (emailStatus.success && regResult.accessKey) {
          await markEmailSent(regResult.accessKey);
        }
      } else {
      ...
      return {
        ...
        accessKey: regResult.accessKey || "",
        expiresAt: regResult.expiresAt || new Date(),
        email: email,
