...
export async function getRecentRegistrations(
  ...
  if (!db) {
    ...
    // Return from cache (cast to CourseRegistration type)
    const cached = registrationCache.getAll();
    if (cached && cached.length > 0) {
      return cached.slice(0, limit) as unknown as CourseRegistration[];
    }
    return [];
  }
  ...
  try {
    ...
    // Fallback to cache (cast to CourseRegistration type)
    const cached = registrationCache.getAll();
    if (cached && cached.length > 0) {
      return cached.slice(0, limit) as unknown as CourseRegistration[];
    }
