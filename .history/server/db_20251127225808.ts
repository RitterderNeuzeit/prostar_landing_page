 ...
 * Handles both Azure MySQL (with SSL) and local MySQL connections.
 */
export async function getDb() {
  if (!_db && !_connectionAttempted && process.env.DATABASE_URL) {
    ...
    try {
      // Remove ssl-mode parameter from URL if present (Drizzle doesn't parse it correctly)
      ...
      if (isAzure) {
        // For Azure, create connection pool with SSL config
        ...
        _db = drizzle(pool);
        console.log("✅ [Database] Connected to Azure MySQL");
      } else {
        // For local MySQL, use URL directly
        _db = drizzle(cleanUrl);
        console.log("✅ [Database] Connected to local MySQL");
      }
    } catch (error) {
      console.warn("⚠️ [Database] Failed to connect:", error instanceof Error ? error.message : error);
      console.log("📝 [Database] Falling back to file-persisted cache");
      _db = null;
    }
  }
  return _db;
