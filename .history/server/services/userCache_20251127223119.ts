...
class UserCache {
  ...
  upsert(user: CachedUser): void {
    ...
    // Check if email already exists with different openId
    if (normalizedEmail) {
      const existingOpenId = this.emailIndex.get(normalizedEmail);
      if (existingOpenId && existingOpenId !== user.openId) {
        // Remove old user entry with same email
        console.log(`🔄 [UserCache] Removing duplicate user ${existingOpenId} for email ${user.email}`);
        this.cache.delete(existingOpenId);
      }
    }
    
    // If user exists by openId, update it
    ...
    if (existing) {
      ...
      if (user.email && user.email !== existing.email) {
        ...
        if (normalizedEmail) {
          this.emailIndex.set(normalizedEmail, user.openId);
        }
      }
    } else {
      // New user
