...
function generatePasswordResetTemplate(data: {
  ...
  const { email, token, expiresAt } = data;
  const siteUrl = process.env.SITE_URL || 'http://localhost:3000';
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <p>wir haben eine Anfrage erhalten, dein Passwort zurückzusetzen. Falls du diese Anfrage nicht gestellt hast, ignoriere diese E-Mail einfach.</p>
            
            <p>Um dein Passwort zurückzusetzen, klicke auf den folgenden Button:</p>
            
            <div style="text-align: center;">
                <a href="${resetUrl}" class="cta-button">Passwort zurücksetzen →</a>
            </div>
            ...
            <div class="warning">
                <strong>⏰ Wichtig:</strong> Dieser Link ist gültig bis ${expiresFormatted} (24 Stunden).
            </div>
            
            <p>Aus Sicherheitsgründen kannst du diesen Link nur einmal verwenden.</p>
            
            <div class="footer">
                <p>Diese E-Mail wurde an ${email} gesendet. Falls du diese E-Mail versehentlich erhalten hast, ignoriere sie bitte.</p>
                <p>© ${new Date().getFullYear()} ProStar Marketing. Alle Rechte vorbehalten.</p>
            </div>
        </div>
    </div>
</body>
...
export async function sendPasswordResetEmail(data: {
  ...
  const htmlContent = generatePasswordResetTemplate(data);
  const textContent = `
Hallo,

wir haben eine Anfrage erhalten, dein Passwort zurückzusetzen.

Um dein Passwort zurückzusetzen, öffne folgenden Link:
${process.env.SITE_URL || 'http://localhost:3000'}/reset-password?token=${encodeURIComponent(data.token)}&email=${encodeURIComponent(data.email)}

Dieser Link ist gültig bis ${data.expiresAt.toLocaleString('de-DE')}.
...
Das ProStar Team
  ...
  console.log(`📧 [Reset] Sending password reset email to ${data.email}...`);
