...
function askQuestion(question, timeoutSec) {
  return new Promise((resolve) => {
    ...
    let answered = false
    const timer = setTimeout(() => { if (!answered) { rl.close(); resolve('') } }, timeoutSec * 1000)
    rl.question(question, (answer) => { answered = true; clearTimeout(timer); rl.close(); resolve(answer.trim()) })
  })
}

async function main() {
  console.log('Automation assisten (node,cjs): Gib eine durch Semikolon getrennte Aufgabenliste ein (wartet', TIMEOUT, 's):')
  const input = await askQuestion('> ', TIMEOUT)
  let tasks
  if (!input) { console.log('Keine Eingabe — verwende Standardaufgaben:', DEFAULT_TASKS.join(', ')); tasks = DEFAULT_TASKS }
  else { tasks = input.split(';').map(s => s.trim()).filter(Boolean) }
...
function runCommandWithTimeout(cmd, timeoutSec) {
  return new Promise((resolve, reject) => {
    ...
    let stdout = ''
    let stderr = ''
    const timer = setTimeout(() => { try { child.kill('SIGTERM') } catch (e) {} ; reject(new Error('timeout')) }, timeoutSec * 1000)
    child.stdout && child.stdout.on('data', (d) => { stdout += d.toString() })
    child.stderr && child.stderr.on('data', (d) => { stderr += d.toString() })
    child.on('error', (err) => { clearTimeout(timer); reject(err) })
    child.on('close', (code) => { clearTimeout(timer); if (code === 0) resolve({ code, stdout: stdout.slice(0,1000) }); else { const e = new Error('non-zero exit'); e.code = code; e.stdout = stdout; e.stderr = stderr; reject(e) } })
  })
}
