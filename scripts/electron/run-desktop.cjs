...
(async function main() {
  try {
    ...
    if (!electronAvailable) {
      console.warn(
        "Electron package not available or not installed properly. Falling back to browser."
      );
      console.log(
        "If you want Electron desktop, run: `pnpm rebuild electron` or `pnpm approve-builds` then reinstall."
      );
    ...
    try {
      ...
      console.log(
        "Falling back to opening the Admin UI in the default browser at",
        DEV_URL
      );
      openInBrowser(DEV_URL);
      process.exit(0);
