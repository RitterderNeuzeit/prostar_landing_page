...
const testimonials: Testimonial[] = [
  {
    ...
    results:
      "LinkedIn Follower: 200 → 1.100 | 12 neue Kunden | €85K zusätzlicher Umsatz",
    verified: true,
    date: "2024-09-15",
  },
  {
  ...
  },
  {
    ...
    results:
      "Instagram Follower: 800 → 3.040 | 25 Anfragen/Monat | €120K zusätzlich",
    verified: true,
    date: "2024-09-01",
  },
  {
...
export default function EnhancedTestimonials({
  ...
  function AvatarImage({ src, name }: AvatarProps) {
    ...
    if (!src) {
      return (
        <div
          className={`rounded-full ${sizeClasses} flex items-center justify-center bg-gradient-to-br from-accent/20 to-accent/10 ring-2 ring-white shadow-sm flex-shrink-0`}
        >
    ...
    // Always try to render the image; let browser handle failures
    return (
      <div
        className={`rounded-full overflow-hidden ${sizeClasses} ring-2 ring-white shadow-sm flex-shrink-0`}
      >
        <img
          src={src}
          alt={name}
  ...
  return (
    <section className={`py-20 md:py-32 ${classes.container}`}>
      <div className="container">
        ...
        >
          {displayedTestimonials.map(testimonial => (
            <motion.div
              ...
              className={`${classes.card} rounded-2xl p-6 md:p-8 shadow-lg hover:shadow-2xl transform transition-all duration-300 hover:scale-[1.025]`}
            >
              {/* Header with Avatar */}
              <div className="flex items-start gap-4 mb-4">
                <AvatarImage src={testimonial.image} name={testimonial.name} />
                <div className="flex-1">
                  <h3 className={`font-semibold ${classes.heading}`}>
                    {testimonial.name}
                  </h3>
                ...
                {testimonial.verified && (
                  <div
                    className={`${classes.badge} px-2 py-1 rounded text-xs font-semibold`}
                  >
                    ✓ Verifiziert
                  </div>
                )}
              ...
              <div className="mb-4">
                <Quote
                  className={`w-5 h-5 ${classes.accent} mb-2 opacity-60`}
                />
                <p
                  className={`italic ${classes.quote} leading-relaxed text-sm`}
                >
                  {testimonial.text}
                </p>
              </div>
              ...
              {testimonial.results && (
                <div
                  className={`mb-4 p-3 rounded ${variant === "professional" ? "bg-gray-100" : variant === "artisanal" ? "bg-[#C85A3A]/10" : "bg-accent/10"}`}
                >
                  <p className={`text-xs font-semibold ${classes.accent} mb-1`}>
                    Ergebnisse:
                  </p>
                  <p className={`text-xs ${classes.results}`}>
                    {testimonial.results}
                  </p>
                </div>
              )}
        ...
        <div
          className={`mt-16 pt-16 border-t ${variant === "professional" ? "border-gray-200" : variant === "artisanal" ? "border-[#C85A3A]/20" : "border-border"}`}
        >
          <h3
            className={`text-3xl font-bold text-center mb-12 ${classes.heading}`}
          >
            Video-Testimonials
          </h3>
            ...
            {[
              ...
              >
                <div className="relative w-full aspect-video bg-gradient-to-br from-accent/20 to-accent/10 flex items-center justify-center group-hover:from-accent/30 group-hover:to-accent/20 transition-colors">
                  <Play
                    className={`w-12 h-12 ${classes.accent} group-hover:scale-110 transition-transform`}
                  />
                </div>
                <div className="p-4">
                  ...
                  <p className={`text-sm ${classes.subheading} mb-2`}>
                    {video.title}
                  </p>
                  <p className={`text-xs ${classes.subheading}`}>
                    {video.duration}
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 text-center">
            <div>
              <div className={`text-3xl font-bold ${classes.accent} mb-2`}>
                500+
              </div>
