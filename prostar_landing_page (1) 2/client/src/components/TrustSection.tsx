...
export default function TrustSection({ variant = 'warm-minimal' }: TrustSectionProps) {
  ...
  const trustBadges = [
    {
      ...
      description: 'Professionell entwickelt und von Branchenexperten validiert',
    },
    {
      icon: Shield,
    ...
    },
    {
      ...
      description: 'Vertraut von Unternehmern in Deutschland, Österreich & Schweiz',
    },
    {
      icon: TrendingUp,
  ...
  const credentials = [
    {
      ...
      description: 'Entwickelt von Social-Media-Profis mit 15+ Jahren Erfahrung',
      icon: '👨‍💼',
    },
    {
      ...
      <div className="container">
        ...
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          <p className={`text-lg ${classes.subheading}`}>
            Transparenz, Qualität und nachgewiesene Ergebnisse sind die Grundlage unseres Kurses.
          </p>
        </div>
          ...
          {trustBadges.map((badge, index) => {
            ...
            return (
              ...
              >
                ...
                <h3 className={`font-semibold ${classes.heading} mb-2`}>{badge.title}</h3>
                <p className={`text-sm ${classes.subheading}`}>{badge.description}</p>
              </div>
            );
          })}
        </div>
        ...
        {/* Stats Section */}
        <div className={`rounded-lg p-8 md:p-12 mb-16 ${variant === 'professional' ? 'bg-white border border-gray-200' : variant === 'artisanal' ? 'bg-[#D4C4B8] border-2 border-[#C85A3A]/20' : 'bg-card'}`}>
          <h3 className={`text-2xl font-bold text-center mb-12 ${classes.heading}`}>
            Nachgewiesene Ergebnisse
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {stats.map((stat, index) => (
              <div key={index} className="text-center">
                <div className={`text-4xl font-bold ${classes.stat} mb-2`}>{stat.number}</div>
                <h4 className={`font-semibold ${classes.heading} mb-1`}>{stat.label}</h4>
                <p className={`text-sm ${classes.subheading}`}>{stat.description}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            {credentials.map((credential, index) => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className={`text-sm ${classes.subheading}`}>{credential.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className={`rounded-lg p-8 md:p-12 ${variant === 'professional' ? 'bg-white border border-gray-200' : variant === 'artisanal' ? 'bg-[#D4C4B8] border-2 border-[#C85A3A]/20' : 'bg-card'}`}>
          <h3 className={`text-2xl font-bold text-center mb-8 ${classes.heading}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={index} className="text-center">
                ...
                <p className={`italic ${classes.subheading} mb-4`}>"{testimonial.quote}"</p>
                <p className={`font-semibold ${classes.heading}`}>{testimonial.author}</p>
                <p className={`text-sm ${classes.subheading}`}>{testimonial.role}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="text-center mt-16">
          ...
          <p className={`max-w-2xl mx-auto ${classes.subheading}`}>
            Wenn Sie nach 30 Tagen nicht vollständig zufrieden sind, erhalten Sie Ihr Geld zurück – ohne Fragen.
            Wir sind so überzeugt von der Qualität unseres Kurses, dass wir dieses Risiko gerne tragen.
          </p>
        </div>
      </div>
    </section>
