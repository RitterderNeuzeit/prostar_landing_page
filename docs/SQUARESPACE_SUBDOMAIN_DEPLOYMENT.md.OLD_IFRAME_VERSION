...
✅ Embedded App wird auf Squarespace-Site angezeigt
...
✅ Session-Cookies werden gesetzt (iframe context beachten!)
✅ Passwort-Reset funktioniert
✅ Mobile Responsive funktioniert
✅ Iframe lädt ohne CORS-Fehler
```obile Responsive funktioniert
```
```bash
# Google PageSpeed Insights
https://pagespeed.web.dev/analysis?url=https://kursprostarmarketing.squarespace.com
...
```bash
# SSL Labs SSL Test
https://www.ssllabs.com/ssltest/analyze.html?d=kursprostarmarketing.squarespace.com

# Ziel: A+ Rating (Squarespace hat bereits A+)
```
# Ziel: A+ Rating
```
...
app.use(
  cors({
    origin: [
      ...
      'https://your-app-xyz.up.railway.app' // Railway Domain
    ],
    credentials: true
  })
);
...
**Für Iframe-Integration besonders wichtig:**

```typescript
// Zusätzlich X-Frame-Options anpassen
app.use((req, res, next) => {
  res.setHeader('X-Frame-Options', 'ALLOW-FROM https://kursprostarmarketing.squarespace.com');
  // Oder für moderne Browser:
  res.setHeader('Content-Security-Policy', "frame-ancestors 'self' https://kursprostarmarketing.squarespace.com https://prostarmarketing.de");
  next();
});
```)
);
...
### Problem: "Session cookies not set in iframe"

**Lösung:**

Cookies in iframes werden oft blockiert (Safari, Chrome mit strict settings). 

**Lösung A: SameSite=None Cookie Attribute**
...
app.use(
  session({
    ...
    cookie: {
      secure: true,
      httpOnly: true,
      sameSite: 'none', // WICHTIG für iframe!
      domain: '.squarespace.com' // Falls auf Squarespace-Domain
    }
  })
);
```

**Lösung B: Token-basierte Auth statt Cookies**

Verwende JWT Tokens in LocalStorage statt Session-Cookies.
...
1. Gehe zu [Stripe Dashboard](https://dashboard.stripe.com/webhooks)
  ...
  gtag('config', 'G-XXXXXXXXXX', {
    'linker': {
      'domains': ['prostarmarketing.de', 'kursprostarmarketing.squarespace.com']
    }
  });
</script>
```
...
Die App läuft embedded in deiner Squarespace-Site und kommuniziert mit:
- ✅ Railway-gehosteter Backend (https://your-app-xyz.up.railway.app)
- ✅ Azure MySQL Database
- ✅ Email-Service (Gmail/SES)
